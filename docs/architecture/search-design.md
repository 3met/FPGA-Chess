...
For threefold repetition, the engine should track hashes for the active game history since the last irreversible move and each search thread should track hashes along its current search line. A search node is drawn if the current position has occurred at least twice earlier in the combined relevant history, making the current occurrence the third. The repetition hash must include side to move, castling rights, and en passant availability. Repetition-draw detection is disabled when `ENABLE_ZOBRIST = 0`.
