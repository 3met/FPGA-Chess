...
For threefold repetition, the engine tracks up to 100 folded 32-bit signatures for the active game history since the last irreversible move and full Zobrist keys along each search thread's current search line. One hundred active-history positions cover every position that can remain relevant before the 50-move draw applies. A search node is drawn if the current position has occurred at least twice earlier in the combined relevant history, making the current occurrence the third. The repetition hash must include side to move, castling rights, and en passant availability. Folding the active-history keys saves registers at the cost of a negligible possibility of a false repetition from a 32-bit collision. Repetition-draw detection is disabled when `ENABLE_ZOBRIST = 0`.
