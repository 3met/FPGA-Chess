...
The NNUE model matches the hardware's direct piece-square feature encoding, accumulator arithmetic, clipped activation, perspective ordering, and quantized output layer. Quantization-aware training uses the same integer ranges and modular accumulator behavior as RTL so exported weights require no rescaling. Checkpoints carry a numerical-model identifier, and incompatible shapes or model versions are rejected.
