...
Search state is stored per configured thread for lifecycle status, scheduler phase, current board, Zobrist key, PST state, pipeline waits, move stacks, alpha/beta values, repetition state, returns, and node counters. Full board states are not stored per ply. Per-node variables are packed into one synchronous block-RAM stack per thread, including actual remaining depth, an 8-bit saturating legal-move count, and node/PVS/recovery flags. Each thread caches its current node in registers, writes that cache through to its current-ply RAM entry, and continuously prefetches the parent entry.
...
TT lookups and stores keep only their issuing thread in `TT_WAIT` or `STORE_WAIT` until the tagged memory completion arrives. Other active threads remain schedulable while an off-chip request is pending. A matching TT best move is retained for targeted move generation even when the stored depth is too shallow for its score or bound to cause a cutoff. All search contexts share the TT: the primary thread orders the previous iteration's principal-variation move first at the root, while helper threads use normal position-dependent legal move ordering and diverge further through shared-TT timing and hits. Shared pipeline arbitration prefers ready primary work, allowing helpers to fill primary stalls without taking a resource from it.
...
Controller-level perft tests cover start-position depths 0, 1, and 2 plus direct-setup positions for kings-only, castling, en passant, promotion, stalemate, and checkmate. Search tests cover White and Black POV capture scoring, 50-move draw, stalemate draw, checkmate losing-mate terminal scores, node limit, fixed-time stop, clock-budget stop, oversized-depth errors, TT reuse, all-thread root scheduling, overlapping tagged pipelines, and depth-4 LMR behavior. LMR checks cover the constant table, non-default Q8 rounding, bucket boundaries and monotonicity, eligibility and clamp rules, reduced and restored child depths, illegal-candidate move-index handling, full-depth recovery, and reduced/full TT request depths.
...
The controller defaults are `SEARCH_THREAD_COUNT = THREAD_COUNT`, `SEARCH_STACK_DEPTH = MAX_PLY_COUNT`, `ACTIVE_REPETITION_DEPTH = 100`, `LMR_A_Q8 = 192`, `LMR_B_Q8 = 614`, and `ENABLE_SEARCH_STATS = 0`. Perft, Zobrist/repetition handling, TT traffic, and incremental PST evaluation are always present.

When `ENABLE_SEARCH_STATS` is set, 40-bit counters record TT lookups and hits, frontend TT-cache lookup probes and hits, and cycles spent in each non-idle per-thread search phase. Exactly one phase counter per thread is enabled each search-run cycle; a shared cycle total supports verification that every thread's phase totals cover the complete scheduler run. The counters reset on every accepted search command, exclude perft and non-search controller work, and are serialized byte-wise through a narrow addressed debug port only after the search. Disabled builds elaborate constant-zero debug values without the counter bank.
...
| `gen_search_stack_ram[].search_stack_mem` | Packed records | One synchronous block-RAM search stack per thread, containing moves, scores, bounds, repetition boundary, actual remaining depth, legal-move count, and node/PVS flags. |
