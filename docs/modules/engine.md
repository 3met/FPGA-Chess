...
The engine parameters configure the controller clock frequency, thread count, stack depth, and whether the TT uses the external-memory backend. Each board wrapper must set `CLOCK_FREQ` to the exact frequency driven on `clk`; clock generation itself stays in the board wrapper so the core remains portable across Intel/Altera and Xilinx devices.
