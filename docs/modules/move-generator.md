...
| Candidate expansion | Priority-selects and clears one source bit, reconstructs the explicit move, attaches attacker/victim/class metadata, and launches a quiet-history read when required. Empty and irrelevant lanes consume no cycles. | Places the candidate in writeback while the walker advances to the next source or destination. |
...
| Completion | Empty destination tails and positions without permitted castles return directly. A pending final write is drained before returning final tagged tops. | Idle/next command. |
