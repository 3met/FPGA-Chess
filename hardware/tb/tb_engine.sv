...
module tb_engine;
    ...
    task automatic test_search_limit_payloads();
        ...
        check(captured.move_overhead == TimeType'(10), "fixed-time move overhead");
        ...
        check(captured.remaining_time == TimeType'(1), "clock search remaining time");
        check(captured.increment == TimeType'(3), "clock search increment");
        check(captured.moves_to_go == 16'd30, "clock search moves to go");
        check(captured.move_overhead == TimeType'(10), "clock search move overhead");
