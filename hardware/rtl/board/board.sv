	...
	always_ff @(posedge clk) begin
		if (~rst_n) begin
			...
			case (board_operation)
				...
				GET_BEST_MOVE_OP:      state <= GET_BEST_MOVE_STATE;
				GET_BOARD_SCORE_OP:    state <= GET_BOARD_SCORE_STATE;
				GET_TILE_OCCUPANCY_OP: state <= GET_TILE_OCCUPANCY_STATE;
				WRITE_TURN_OP:         state <= WRITE_TURN_STATE;
				WRITE_CASTLE_OP:       state <= WRITE_CASTLE_STATE;
