...
module tt_memory_cdc_bridge #(
    ...
    input logic req_clk, input logic req_rst_n,
    input logic mem_clk, input logic mem_rst_n,
    input logic backend_ready, input logic backend_error,
    output logic req_memory_ready, output logic req_memory_error,
    ...
    output logic backend_req_valid, input logic backend_req_ready, output logic backend_req_write,
    output TTWordAddress backend_req_address, output logic [3:0] backend_req_length,
    output logic backend_write_valid, input logic backend_write_ready,
    output logic [15:0] backend_write_data, output logic backend_write_last,
    input logic backend_read_valid, output logic backend_read_ready,
    input logic [15:0] backend_read_data, input logic backend_read_last,
    input logic backend_done_valid, output logic backend_done_ready, input logic backend_done_error
);
    ...
    logic [16:0] write_fifo_data;
    logic read_full, read_empty;
    logic [16:0] read_fifo_data;
    ...
    async_fifo #(.DATA_WIDTH(17), .DEPTH(WRITE_FIFO_DEPTH)) write_fifo (
    ...
    async_fifo #(.DATA_WIDTH(17), .DEPTH(READ_FIFO_DEPTH)) read_fifo (
