...
When `--port` is omitted, the host uses `FPGA_CHESS_PORT` or attempts to identify a suitable USB UART. The FPGA command and response format is specified in [Laptop-FPGA Communication](docs/protocols/laptop-fpga-communication.md).
